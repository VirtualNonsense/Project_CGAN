...
class Discriminator(nn.Module):
    ...
    def forward(self, x, y):
        # pass the labels into a embedding layer
        labels_embedding = self.embedding(y).view(y.shape[0], 3* 64*64)
...
if __name__ == "__main__":
    ...
    trainer = pl.Trainer(max_epochs=10, gpus=1 if torch.cuda.is_available() else 0, progress_bar_refresh_rate=50)
